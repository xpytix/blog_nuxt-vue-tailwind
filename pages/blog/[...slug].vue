...
// Jeśli `queryCollection` jest globalne, nie potrzebujesz importu.
// Jeśli jest częścią modułu, upewnij się, że jest dostępne.
// `useAsyncData` jest auto-importowane przez Nuxt 3.
...
// Interfejs dla danych strony - DOSTOSUJ DO STRUKTURY ZWRACANEJ PRZEZ TWOJE `queryCollection`
interface PageData {
  title: string;
  date?: string | Date;
  author?: string;
...
const { data: page, pending, error } = await useAsyncData<PageData>(
  ...
  async () => {
    console.log(`[pages/blog/...slug.vue] Próba wykonania queryCollection dla path: ${route.path}`);
    try {
      // Używamy DOKŁADNIE Twojego zapytania
      // @ts-ignore: Ignorujemy błąd TS dla queryCollection, zakładając, że istnieje i działa
      const result = await queryCollection('blog').path(route.path).first();
      
      if (!result) {
        console.warn(`[pages/blog/...slug.vue] queryCollection nie zwróciło danych dla: ${route.path}`);
        // Rzucenie błędu spowoduje wyświetlenie strony błędu Nuxt
        throw createError({ statusCode: 404, statusMessage: 'Strona nie została znaleziona', fatal: true });
      }
      console.log(`[pages/blog/...slug.vue] Sukces! Pobrano dane strony:`, JSON.parse(JSON.stringify(result)));
      return result;
    } catch (e: any) {
      console.error(`[pages/blog/...slug.vue] BŁĄD w queryCollection:`, e.message, e);
